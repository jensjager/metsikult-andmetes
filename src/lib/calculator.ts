...
export const SATELLITE_AUDIT_CONFIG = {
  ...
  THINNED_WOOD_VALUE_REMAINING: 0.60, // Allesjääv puidu väärtus pärast harvendust (60%)
  THINNED_WOOD_VOLUME_LOST: 0.40,     // Eemaldatav puistu maht kokku pärast harvendust (40%)
  ...
  CLEARCUT_NDVI_BASE: 0.20,           // Lageraie tegelik baas-NDVI
  CLEARCUT_NDPI_BASE: 0.32,           // Lageraie raiesmiku baas-NDPI (niiskus)
  THINNED_NDVI_BASE: 0.50,            // Osalise raie tegelik baas-NDVI
...
function getSatelliteAuditForEraldis(
  ...
  const inventYear = inventKuup ? new Date(inventKuup).getFullYear() : currentYear;
  const staleYears = isNaN(inventYear) ? 0 : Math.max(0, currentYear - inventYear);
  ...
  const clearcutThreshold = Math.min(config.CLEARCUT_RISK_MAX, config.CLEARCUT_BASE_RISK + staleYears * config.CLEARCUT_RISK_SCALE_PER_YEAR);
  const thinnedThreshold = clearcutThreshold + Math.min(config.THINNED_RISK_MAX, config.THINNED_BASE_RISK + staleYears * config.THINNED_RISK_SCALE_PER_YEAR);
  ...
  // Hõrendus/harvendus
    ...
    ndviExpected = 0.80 + (absHash % 5) / 100;
...
export function calculateForestValue(
  ...
  for (const eraldis of eraldised) {
    ...
    const katastriNr = props.katastritunnus || props.katastri_nr || eraldised[0]?.properties?.katastritunnus || eraldised[0]?.properties?.katastri_nr || "Teadmata";
  ...
  if (hasMissingData) {
    warning = "Mõne eraldise puhul puudus registrist detailne puuliikide info. Nende väärtus arvutati lihtsustatult.";
  }
  if (auditPeriod === 'active' && hasClearcut) {
    warning = "Satelliitkontrolli hoiatus: Tuvastati lageraie märke mõnel eraldise alal! Koguväärtust on tegeliku olukorra alusel vähendatud.";
  }
